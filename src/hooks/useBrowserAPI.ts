    ...
    try {
      ...
      if (typeof window !== 'undefined' && window.require) {
        ...
        const browserAPI: BrowserAPI = {
          ...
          navigateTab: (viewId: string, url: string) => ipcRenderer.invoke('navigate-tab', viewId, url),
        ...
        const mockAPI: BrowserAPI = {
          createTab: async (url?: string) => ({ id: `mock-${Date.now()}`, url: url || 'https://google.com', title: 'Mock Tab' }),
          switchTab: async (viewId: string) => { console.log('Mock switch to:', viewId); return true; },
          closeTab: async (viewId: string) => { console.log('Mock close:', viewId); return true; },
          navigateTab: async (viewId: string, url: string) => { console.log('Mock navigate:', viewId, url); return true; },
          getTabInfo: async (viewId: string) => ({ url: 'https://example.com', title: 'Mock Tab', canGoBack: false, canGoForward: false }),
        };
      ...
      const fallbackAPI: BrowserAPI = {
        ...
        getTabInfo: async () => ({ url: 'about:blank', title: 'Fallback', canGoBack: false, canGoForward: false }),
      };
      setApi(fallbackAPI);
    }
  }, []);
